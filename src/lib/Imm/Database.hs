...
describeDatabase :: (MonadFree (SumF f) m, DatabaseF t :<: f)
                 => t -> m Doc
describeDatabase t = liftF . inj $ Describe t id

fetch :: (MonadFree (SumF f) m, DatabaseF t :<: f, Table t, MonadThrow m)
      => t -> Key t -> m (Entry t)
fetch t k = do
...
fetchList :: (MonadFree (SumF f) m, DatabaseF t :<: f, MonadThrow m)
          => t -> [Key t] -> m (Map (Key t) (Entry t))
fetchList t k = do
  result <- liftF . inj $ FetchList t k id
  liftE result

fetchAll :: (MonadThrow m, MonadFree (SumF f) m, DatabaseF t :<: f) => t -> m (Map (Key t) (Entry t))
fetchAll t = do
  result <- liftF . inj $ FetchAll t id
  liftE result

update :: (MonadFree (SumF f) m, DatabaseF t :<: f, MonadThrow m)
       => t -> Key t -> (Entry t -> Entry t) -> m ()
update t k f = do
  result <- liftF . inj $ Update t k f id
  liftE result

insert :: (MonadThrow m, MonadFree (SumF f) m, LoggerF :<: f, DatabaseF t :<: f)
       => t -> Key t -> Entry t -> m ()
insert t k v = insertList t [(k, v)]

insertList :: (MonadThrow m, MonadFree (SumF f) m, LoggerF :<: f, DatabaseF t :<: f)
           => t -> [(Key t, Entry t)] -> m ()
insertList t i = do
...
delete :: (MonadThrow m, MonadFree (SumF f) m, LoggerF :<: f, DatabaseF t :<: f) => t -> Key t -> m ()
delete t k = deleteList t [k]

deleteList :: (MonadThrow m, MonadFree (SumF f) m, LoggerF :<: f, DatabaseF t :<: f)
           => t -> [Key t] -> m ()
deleteList t k = do
...
purge :: (MonadThrow m, MonadFree (SumF f) m, DatabaseF t :<: f, LoggerF :<: f) => t -> m ()
purge t = do
  logInfo "Purging database..."
  result <- liftF . inj $ Purge t id
  liftE result

commit :: (MonadThrow m, MonadFree (SumF f) m, DatabaseF t :<: f, LoggerF :<: f) => t -> m ()
commit t = do
